...
{{-- input type text --}}
@if ($input->type == 1)
<form class="ui-state-default" action="{{route('admin.reservation.inputDelete')}}" method="post" data-id="{{$input->id}}" data-id="{{$input->id}}">
    {{csrf_field()}}
    ...
    <div class="row">
        ...
        <div class="d-flex align-items-center col-md-2 col-sm-12 mt-2">
        <a class="btn btn-warning btn-sm" href="{{route('admin.reservation.inputEdit', $input->id) . '?language=' . request()->input('language')}}" target="_blank">
...
@elseif ($input->type == 7)
<form class="ui-state-default" action="{{route('admin.reservation.inputDelete')}}" method="post" data-id="{{$input->id}}">
    {{csrf_field()}}
    <input type="hidden" name="input_id" value="{{$input->id}}">
    <div class="form-group">
    <label for="">{{$input->label}} @if($input->required == 1) <span>**</span> @elseif($input->required == 0) (Optional) @endif</label>
