      ...
      <div class="card">
        <div class="card-header">
          <div class="card-title">
              <div class="row">
                  <div class="col-lg-6">
                    {{__('Edit')}}: {{__('Input Fields')}}
                  </div>
                  <div class="col-lg-6 text-right">
                    <a class="btn btn-primary" href="{{route('admin.reservation.form') . '?language=' . request()->input('language')}}">{{__('back')}}</a>
                  </div>
              </div>
          </div>
        </div>
                ...
                <div class="col-lg-6 offset-lg-3">
                    <form id="ajaxForm" action="{{route('admin.reservation.inputUpdate')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="">
                            <input type="text" class="form-control" name="label" value="{{$input->label}}" placeholder="{{__('Name')}}...">
                            </div>
                        ...
                        @if ($input->type != 3)
                            <div class="form-group">
                                ...
                                <div class="">
                                    <input type="text" class="form-control" name="placeholder" value="{{$input->placeholder}}" placeholder="{{__('Placeholder')}}">
                                </div>
                        ...
                        @if ($input->type == 2 || $input->type == 3)
                            <div class="form-group">
                                ...
                                <div class="col-md-11">
                                    <input class="form-control optionin" type="text" name="options[]" :value="names[n-1]" placeholder="{{__('Options')}}...">
                                </div>
                                ...
                                <button type="button" class="btn btn-success btn-sm text-white" @click="addOption()"><i class="fa fa-plus"></i> {{__('add new')}}: {{__('Options')}}</button>
                        ...
                        <button id="submitBtn" type="submit" class="btn btn-primary">{{__('Update')}}</button>
                        </div>
                    </form>
                </div>
            </div>
