                    ...
                    <div class="card-body px-2">
                        <form id="orderForm" action="{{route('admin.pos.placeOrder')}}" method="POST">
                            ...
                            <div class="form-group p-0 pb-2">
                                <div class="ui-widget">
                                    <label for="">{{__('Billing Phone')}}</label>
                                    <input class="form-control" type="text" name="customer_phone" placeholder="{{__('Billing Phone')}}..." value="{{old('customer_phone')}}" onchange="loadCustomerName(this.value)">
                                </div>
                            </div>
                             -->
                             <!--
                            <div class="form-group p-0 pb-2">
                                <div class="ui-widget">
                                    <label for="">{{__('Billing Last Name')}}</label>
                                    <input class="form-control" name="customer_name" type="text" placeholder="{{__('Billing Last Name')}}" value="{{old('customer_name')}}" disabled>
                            ...
                            <div class="form-group p-0 pb-2">
                                ...
                                <select class="form-control" name="serving_method" required>
                                    @foreach ($smethods as $smethod)
                                        <option value="{{$smethod->value}}" {{$smethod->value == old('serving_method') ? 'selected' : ''}}>{{__($smethod->name)}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <!--
                            <div class="form-group p-0 pb-2">
                                ...
                                <select class="form-control select2" name="payment_method">
                                    <option value="" selected disabled>{{__('Payment Method')}}</option>
                                    @foreach ($pmethods as $pmethod)
                                        <option value="{{$pmethod->name}}" {{$pmethod->name == old('payment_method') ? 'selected' : ''}}>{{__($pmethod->name)}}</option>
                                    @endforeach
                                </select>
                            </div>
                            -->
                            ...
                            <div id="on_table" class="d-none extra-fields">
                                <div class="form-group p-0 pb-2">
                                    ...
                                    <select class="form-control select2" name="table_no">
                                        <option value="" selected disabled>{{ __('ChooseTableText')}}</option>
                            ...
                            <div id="pick_up" class="d-none extra-fields">
                                <div class="form-group p-0 pb-2">
                                    <label for="">{{__('Pick up Date')}}</label>
                                    <input name="pick_up_date" type="text" class="form-control datepicker" id="pickup-date" placeholder="{{__('Pick up Date')}}" autocomplete="off">
                                    <script>
                                ...
                                <div class="form-group p-0 pb-2">
                                    <label for="">{{__('Pick up Time')}}</label>
                                    <input name="pick_up_time" type="text" class="form-control timepicker" placeholder="{{__('Pick up Time')}}" autocomplete="off">
                                </div>
                            </div>
                            ...
                            <div id="home_delivery" class="d-none extra-fields">
                                @if ($be->delivery_date_time_status == 1)
                                    ...
                                    <div class="form-group p-0 pb-2">
                                        ...
                                        <select id="deliveryTime" class="form-control" name="delivery_time" disabled>
                                            <option value="" selected disabled>{{__('Delivery Time')}}</option>
                                        </select>
                                    </div>
                                @endif
                                @if ($bs->postal_code == 0)
                                ...
                                @else
                                    <div class="form-group p-0 pb-2">
                                        <label>{{__('Post Code')}} (Delivery Area)</label>
            ...
            <div class="col-lg-5">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-6">
                                <h4 class="text-white">{{__('Ordered Products')}}</h4>
                            </div>
                        </div>
                                ...
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" >{{__('Price')}} ({{$be->base_currency_symbol}})</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                    ...
                    <div class="card-footer text-center">
                        <div class="row g-lg-1">
                            ...
                            <div class="col-md-4 col-sm-12 mb-2">
                                <button class="btn btn-success btn-block" id="printBtn"><i class="fas fa-print"></i> {{__('Print')}}</button>
                            </div>
                            <div class="col-md-4 col-sm-12 mb-2">
                                <button class="btn btn-danger btn-block" id="clearCartBtn">{{__('Clear Cart')}}</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{__('Calculator')}}</h5>
...
function setShippingCharge() {
    ...
    if(servingMethod == 'home_delivery') {
        if(postalCode == 0) {
            ...
            charge = $("select[name='postal_code']").children('option:selected').attr('data');
        }
    } else {
        charge = 0.00;
    }
