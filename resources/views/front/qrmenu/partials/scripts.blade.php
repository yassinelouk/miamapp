<script>
    ...
    function useFidelityPoints() {
        $.post(
            "{{route('front.usefidelity')}}",
            function(data) {
                if(data.status == 'success') {
                    toastr["success"](data.message);
                    if(data.btn_state) {
                        $('#fidelity-btn').replaceWith(data.btn_state);
                    }
                    $("#cartTotal").load(location.href + " #cartTotal", function() {
                        calcTotal();
                    });
    ...
    $(document).ready(function() {
        ...
        showOfflineGateways();
        $('#sendOrderBtn').on('click',function(e) {
            e.preventDefault();
            $('#payment').attr('action',"{{route('product.payment.qrsaveorder', 'open')}}");
            $('#payment').submit();
        })
        $('#placeOrderBtn').on('click',function(e) {
            e.preventDefault();
            $('#payment').attr('action',"{{route('product.payment.qrsaveorder','close')}}");
            $('#payment').submit();
        })
        ...
        if(val == 'home_delivery') {
            $('.shipping_charge_price').show();
        } else {
            $('.shipping_charge_price').hide();
        }
