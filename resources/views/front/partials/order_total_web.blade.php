<div class="cart-total" id="orderTotal">
    ...
    <div id="cartTotal">
        <ul class="cart-total-table">
            ...
            <li class="shipping_charge_price">
    ...
    <div class="payment-options">
        <span>@includeIf('front.product.payment-gateways')</span>
        @error('gateway')
        <p class="text-danger mb-0">{{ convertUtf8($message) }}</p>
        @enderror
        <div class="placeorder-button {{$rtl == 1 ? 'text-right' : 'text-left'}} mt-4">
                        <button class="main-btn" type="submit" form="payment" id="placeOrderBtnWeb"><span
                            class="btn-title"><i class="fas fa-receipt"></i> {{ __('Checkout') }}</span></button>
                    </div>
    </div>
</div>
