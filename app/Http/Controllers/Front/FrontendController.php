...
class FrontendController extends Controller
{
    ...
    public function tableBook(Request $request)
    {
        ...
        Session::flash('success', 'Requête de réservation envoyée. Nous vous contacterons dans les plus brefs délais !');
        return back();
    }
    ...
    public function JobApplicationStore(Request $request)
    {
        ...
        $rules = [
            ...
            'content' => 'required',
            'cv' => [
                    function ($attribute, $value, $fail) use ($cv, $allowedExts) {
                        if (!empty($cv)) {
                            $ext = $cv->getClientOriginalExtension();
                            if (!in_array($ext, $allowedExts)) {
                                return $fail("Only doc, pdf, docx files are allowed");
                            }
                        }
                    },
                ],
        ...
        $jresponse = new Japplications;
        $jresponse['full_name'] = $request->full_name;
        $jresponse['subject'] = $request->title;
        $jresponse['slug'] = $slug;

        if($request->hasFile('cv')){
            $filename = time() . '.' . $cv->getClientOriginalExtension();
            $request->session()->put('cv_file', $filename);
            $request->file('cv')->move('assets/admin/cvs/', $filename);
            $jresponse['cv_name'] = $filename;
        }
        $jresponse['content'] = Purifier::clean($request->content);
        $jresponse['language_id'] = $lang->id;
